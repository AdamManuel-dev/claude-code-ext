...
class MCPSimulatorServer {
  ...
  constructor() {
    // Create MCP server without initializing the orchestrator
  ...
  private registerTools() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      if (request.params.name === "process-instruction") {
        ...
        try {
          const result = await this.orchestrator.processInstruction(
            instruction
          );
  ...
  async start() {
    ...
    try {
      // Initialize orchestrator after full module initialization to break the circular dependency
      const serverComponents = await createMCPServer();
      this.orchestrator = serverComponents.orchestrator;

      // Register tools now that orchestrator is initialized
      this.registerTools();
